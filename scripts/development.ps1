...
Get-ChildItem "$AppPath\src" -Directory | ForEach-Object {
    ...
    $meta = Load-Metadata -MetaFile $MetadataFile
    ...
    if (Validate-Compose -ComposeFile $ComposeFile -ServiceName $serviceName) {
        Deploy-Service -ServiceName $serviceName -ComposeFile $ComposeFile
    }
}
