...
scp -o StrictHostKeyChecking=no deploy/scripts/initialize-remote-server.sh root@"$REMOTE_IP":/tmp/app/remote-swarm-init.sh
...
ssh -o StrictHostKeyChecking=no root@"$REMOTE_IP" bash <<EOF
  chmod +x /tmp/app/remote-swarm-init.sh
  export PRIVATE_IP="$PRIVATE_IP"
  export MANAGER_IP="$MANAGER_IP"
  /tmp/app/remote-swarm-init.sh
EOF
