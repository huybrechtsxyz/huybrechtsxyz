...
public class CopyModel : PageModel
{
    ...
    private readonly IValidator<Flow.CopyQuery> _getValidator;
    private readonly IValidator<Flow.CopyCommand> _postValidator;

    [BindProperty]
    public Flow.CopyCommand Data { get; set; }

    [TempData]
    public string StatusMessage { get; set; } = string.Empty;

    public CopyModel(IMediator mediator,
        IValidator<Flow.CopyQuery> getValidator,
        IValidator<Flow.CopyCommand> postValidator)
    {
        _mediator = mediator;
    ...
    public async Task<IActionResult> OnGetAsync(Ulid id)
    {
        try
        {
            Flow.CopyQuery message = new() { Id = id };
