...
public class CreateModel : PageModel
{
    ...
    public async Task<IActionResult> OnGetAsync(Ulid platformRateId)
    {
        try
        {
            Flow.CreateQuery message = new() { PlatformRateId = platformRateId };
            ...
            if (result.IsFailed)
                return RedirectToPage(nameof(Index), new { platformRateId });
    ...
    public async Task<IActionResult> OnPostAsync()
    {
        try
        {
            ...
            if (!state.IsValid)
            {
                ...
                var refresh = await _mediator.Send(new Flow.CreateQuery() { PlatformRateId = Data.PlatformRateId });
