...
namespace Huybrechts.Web.Attributes;
...
public sealed partial class AppPhoneAttribute : DataTypeAttribute
{
    ...
    [GeneratedRegex(@"^(\+\s?)?((?<!\+.*)\(\+?\d+([\s\-\.]?\d+)?\)|\d+)([\s\-\.]?(\(\d+([\s\-\.]?\d+)?\)|\d+))*(\s?(x|ext\.?)\s?\d+)?$", RegexOptions.IgnoreCase | RegexOptions.ExplicitCapture | RegexOptions.Compiled, "en-US")]
    ...
    private static string RemoveExtension(string potentialPhoneNumber)
    {
        ...
        {
            string extension = potentialPhoneNumber[(lastIndexOfExtension + 4)..];
            if (MatchesExtension(extension))
            {
                return potentialPhoneNumber[..lastIndexOfExtension];
            }
        }
        ...
        if (lastIndexOfExtension >= 0)
        {
            string extension = potentialPhoneNumber[(lastIndexOfExtension + 3)..];
            if (MatchesExtension(extension))
            {
                return potentialPhoneNumber[..lastIndexOfExtension];
            }
        ...
        if (lastIndexOfExtension >= 0)
        {
            string extension = potentialPhoneNumber[(lastIndexOfExtension + 1)..];
            if (MatchesExtension(extension))
            {
                return potentialPhoneNumber[..lastIndexOfExtension];
            }
