...
namespace Huybrechts.App.Features.Platform.PlatformRegionFlow;

public static class PlatformRegionHelper
{
    ...
    internal static Result RecordNotFound(Ulid id) => Result.Fail(Messages.INVALID_PLATFORMREGION_ID.Replace("{0}", id.ToString()));

    internal static Result DuplicateRecordFound(string name) => Result.Fail(Messages.DUPLICATE_PLATFORMREGION_NAME.Replace("{0}", name.ToString()));
...
internal sealed class ListHandler :
    EntityFlow.ListHandler<PlatformRegion, ListModel>,
    IRequestHandler<ListQuery, Result<ListResult>>
{
    ...
    public async Task<Result<ListResult>> Handle(ListQuery message, CancellationToken token)
    {
        ...
        if (platform == null)
            return PlatformRegionHelper.PlatformNotFound(message.PlatformInfoId ?? Ulid.Empty);
...
public sealed class CreateCommandValidator : ModelValidator<CreateCommand>
{
    public CreateCommandValidator(FeatureContext dbContext) : base()
    {
        ...
        {
            bool exists = await PlatformRegionHelper.IsDuplicateNameAsync(dbContext, model.Name, model.PlatformInfoId);
            return !exists;
        })
...
internal class CreateQueryHandler : IRequestHandler<CreateQuery, Result<CreateCommand>>
{
    ...
    public async Task<Result<CreateCommand>> Handle(CreateQuery message, CancellationToken token)
    {
        ...
        if (platform is null)
            return PlatformRegionHelper.PlatformNotFound(message.PlatformInfoId);

        var record = PlatformRegionHelper.CreateNew(platform);
...
internal sealed class CreateCommandHandler : IRequestHandler<CreateCommand, Result<Ulid>>
{
    ...
    public async Task<Result<Ulid>> Handle(CreateCommand message, CancellationToken token)
    {
        ...
        if (await PlatformRegionHelper.IsDuplicateNameAsync(_dbcontext, message.Name, message.PlatformInfoId))
            return PlatformRegionHelper.DuplicateRecordFound(message.Name);
...
public class UpdateCommandValidator : ModelValidator<UpdateCommand> 
{
    public UpdateCommandValidator(FeatureContext dbContext)
    {
        ...
        RuleFor(x => x).MustAsync(async (model, cancellation) =>
        {
            bool exists = await PlatformRegionHelper.IsDuplicateNameAsync(dbContext, model.Name, model.PlatformInfoId, model.Id);
            return !exists;
        })
        .WithMessage(x => Messages.DUPLICATE_PLATFORMREGION_NAME.Replace("{0}", x.Name.ToString()))
        .WithName(nameof(CreateCommand.Name));
...
internal class UpdateQueryHandler : IRequestHandler<UpdateQuery, Result<UpdateCommand>>
{
    ...
    public async Task<Result<UpdateCommand>> Handle(UpdateQuery message, CancellationToken token)
    {
        ...
        if (record == null) 
            return PlatformRegionHelper.RecordNotFound(message.Id);

        var platform = await _dbcontext.Set<PlatformInfo>().FindAsync([record.PlatformInfoId], cancellationToken: token);
        if (platform is null)
            return PlatformRegionHelper.PlatformNotFound(record.PlatformInfoId);
        //else
...
internal class UpdateCommandHandler : IRequestHandler<UpdateCommand, Result>
{
    ...
    public async Task<Result> Handle(UpdateCommand message, CancellationToken token)
    {
        ...
        if (await PlatformRegionHelper.IsDuplicateNameAsync(_dbcontext, message.Name, message.PlatformInfoId, record.Id))
            return PlatformRegionHelper.DuplicateRecordFound(message.Name);
    ...
    IRequestHandler<ImportQuery, Result<ImportResult>>
{
    ...
    public async Task<Result<ImportResult>> Handle(ImportQuery message, CancellationToken token)
    {
        ...
        var searchString = message.SearchText ?? message.CurrentFilter;
        List<ImportModel> records = await GetAzureLocationsAsync(platform.Provider.ToString(), message.PlatformInfoId, searchString);
