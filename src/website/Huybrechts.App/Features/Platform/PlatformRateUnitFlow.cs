...
namespace Huybrechts.App.Features.Platform.PlatformRateUnitFlow;
public static class PlatformRateUnitHelper
{
    public static CreateCommand CreateNew(PlatformRate rate) => new()
    {
        Id = Ulid.NewUlid(),
    ...
    internal static Result RateNotFound(Ulid id) => Result.Fail(Messages.INVALID_PLATFORMRATE_ID.Replace("{0}", id.ToString()));

    internal static Result UnitNotFound(Ulid id) => Result.Fail(Messages.INVALID_SETUPUNIT_ID.Replace("{0}", id.ToString()));

    internal static Result RecordNotFound(Ulid id) => Result.Fail(Messages.INVALID_PLATFORMRATEUNIT_ID.Replace("{0}", id.ToString()));
}
    ...
    IRequestHandler<ListQuery, Result<ListResult>>
{
    ...
    public async Task<Result<ListResult>> Handle(ListQuery message, CancellationToken token)
    {
        ...
        if (rate == null)
            return PlatformRateUnitHelper.RateNotFound(message.PlatformRateId ?? Ulid.Empty);
        ...
        if (product == null)
            return PlatformRateUnitHelper.ProductNotFound(rate.PlatformProductId);

        var platform = await _dbcontext.Set<PlatformInfo>().FirstOrDefaultAsync(q => q.Id == rate.PlatformInfoId, cancellationToken: token);
        if (platform == null)
            return PlatformRateUnitHelper.PlatformNotFound(rate.PlatformInfoId);
...
internal class CreateQueryHandler : IRequestHandler<CreateQuery, Result<CreateCommand>>
{
    ...
    public async Task<Result<CreateCommand>> Handle(CreateQuery message, CancellationToken token)
    {
        ...
        if (rate is null)
            return PlatformRateUnitHelper.ProductNotFound(message.PlatformRateId);
        ...
        var record = PlatformRateUnitHelper.CreateNew(rate);
...
internal sealed class CreateCommandHandler : IRequestHandler<CreateCommand, Result<Ulid>>
{
    ...
    public async Task<Result<Ulid>> Handle(CreateCommand message, CancellationToken token)
    {
        var rate = await _dbcontext.Set<PlatformRate>().FindAsync([message.PlatformRateId], cancellationToken: token);
        if (rate is null)
            return PlatformRateUnitHelper.PlatformNotFound(message.PlatformRateId);

        var unit = await _dbcontext.Set<SetupUnit>().FindAsync([message.SetupUnitId], cancellationToken: token);
        if (unit is null)
            return PlatformRateUnitHelper.UnitNotFound(message.SetupUnitId);
