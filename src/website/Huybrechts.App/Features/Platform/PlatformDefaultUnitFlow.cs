...
public static class PlatformDefaultUnitFlow
{
    ...
    public record Model
    {
        ...
        public string SearchIndex => GetSearchIndex(UnitOfMeasure, SetupUnit.Name, Description);
    }
    ...
    private static Result RecordNotFound(Ulid id) => Result.Fail(Messages.INVALID_PLATFORMDEFAULTUNIT_ID.Replace("{0}", id.ToString()));
        ...
        public async Task<Result> Handle(ImportCommand message, CancellationToken token)
        {
            ...
            foreach (var item in message.Items)
            {
                PlatformDefaultUnit record = new()
                {
                    ...
                    UnitOfMeasure = item.UnitOfMeasure,
                    SearchIndex = GetSearchIndex(item.UnitOfMeasure, item.SetupUnit.Name, item.UnitOfMeasure)
                };
